...
**FreeCodeCamp에 기재된 [Preethi Kasireddy의 포스트](https://medium.freecodecamp.com/javascript-modules-part-2-module-bundling-5020383cf306#.kthd3yfad)를 번역한 글입니다. 이해한대로 의역했기 때문에 오역 및 빠뜨리는 부분이 있을 수 있습니다.**
...
Browserify는 해당 프로젝트의 의존성을 파악하기 위해서 파싱된 [AST](https://en.wikipedia.org/wiki/Abstract_syntax_tree)를 돌아다닙니다.
...
CommonsJS vs AMD에 관심이 있다면 [Tom Dale's blog](http://tomdale.net/2012/01/amd-is-not-the-answer/)를 읽어보세요:)
...
죽은 코드를 지우는것(dead code elimination)이 가끔은 UglifyJS와 ES6사이에서 정확히 같은 일을 수행 할수도 있고 아닐 수 도 있습니다.[Rollup’s wiki](https://github.com/rollup/rollup)를 보시면 됩니다.
...
tree shaking을 좀더 잘 알기 위해서는 [http://rollupjs.org/](http://rollupjs.org/)를 참고해 보세요.
...
2. [Rollup.js](http://rollupjs.org/)
   을 사용하는것은 번들링전에 ES6코드와 의존성을 정적으로 분석하는것 외에는 첫번째
   방법과 유사합니다. 이는 번들에 포함되는 것을 최소화하기 위해 tree shaking을 사용
   합니다. 결국 Rollup.js를 사용하는것의 가장 큰 장점은 tree shaking을 통해서 번들을
   최소화 해주는겁니다. 주의해야 할 점은 Rollup은 코드를 번들하기 위한 ES6, CommonJS,
   AMD, UMD, IIFE같은 형식을 지원한다는 것입니다. 만약 AMD, CommonJS, ES6를 번들하기로
   정했는데 IIFE, UMD가 그대로 브라우저에서 작동한다면 브라우저가 이해할 수 있는
   방법으로 바꾸는 다른 메소드를 찾아야 합니다.(Browserify, Webpack, RequireJS 등등)
...
ECMAScript는 현재 [ECMAScript6 module loader API](https://github.com/ModuleLoader/es6-module-loader/) 라고하는 해답을 위한 설명서를 가지고 있습니다. 간단히 말해서 프로그램입니다. Promise-based API는 동적으로 모듈을 로드하고 이후에 새로운 모듈을 다시 불러오지 않기 위해 그걸 캐싱해놓습니다.
...
아직 [module loader API Polyfill의 repo](https://github.com/ModuleLoader/es6-module-loader/)를 꼭 읽어보시기를 권장합니다.

추가적으로 테스트해보고 싶으시다면 ES6 Module Loader polyfill의 상단에 있는 [System.js](https://github.com/systemjs/systemjs)를 check out하세요.
...
PS. HTTP/2의 다른 차이가 궁금하다면 [이곳](https://http2.github.io/faq/#what-are-the-key-differences-to-http1x)을 읽어보세요.
